                          ...
                          const Real frequency)
{
  ...
  for (unsigned int n=1; n<N+1; n++)
  {
    ...
    double alfa = ep[n-1][3]/180.*pi;
    double beta = ep[n-1][4]/180.*pi;
    double gama = ep[n-1][5]/180.*pi;
    ...
    cond_(0,0) = ep[n-1][0];      cond_(0,1) = 0.;            	cond_(0,2) = 0.;
    cond_(1,0) = 0.;              cond_(1,1) = ep[n-1][1];      cond_(1,2) = 0.;
    cond_(2,0) = 0.;              cond_(2,1) = 0.;            	cond_(2,2) = ep[n-1][2];

    this->sigma[n] = (Rz*Rx*R_z*cond_)*(R_z.transpose()*Rx.transpose()*Rz.transpose());

    // For epsilon
    this->epsilon[n].setZero();
    epsilon[n](0,0) = ep[n-1][6];     epsilon[n](0,1) = ep[n-1][7];     epsilon[n](0,2) = ep[n-1][8];
    epsilon[n](1,0) = ep[n-1][9];     epsilon[n](1,1) = ep[n-1][10];    epsilon[n](1,2) = ep[n-1][11];
    epsilon[n](2,0) = ep[n-1][12];    epsilon[n](2,1) = ep[n-1][13];    epsilon[n](2,2) = ep[n-1][14];
    ...
    // For impedivity
    const double mu = ep[n-1][15]*EM::mu0;
    this->z_hat[n] = EM::II*omega*mu;

    // For depth
    this->depth[n] =  ep[n-1][16];
  }

  // Computing thickness of each layer h
...
// compute impedance at given l-th layer's surface.
...
void AS::post_process(std::ofstream& out)
{
  ...
  App.setZero();
  Phase.setZero();
  double abs_real_Z0=0., abs_imag_Z0=0.;
  for(unsigned int i=0; i<2; i++)
  {
    for(unsigned int j=0; j<2; j++)
    {
        ...
        // For phase
	abs_real_Z0 = std::abs(Z[0](i,j).real());
	abs_imag_Z0 = std::abs(Z[0](i,j).imag());

	if( ( abs_real_Z0 < TOL ) && ( abs_imag_Z0 < TOL ) ) // Z0==0
	  Phase(i,j) = 0.;
        else if( ( abs_imag_Z0 < TOL ) && ( Z[0](i,j).real() < 0.0 ) )
          Phase(i,j) = 180.;
	else
  ...
  out << setiosflags(std::ios::left) << std::setw(16) << this->f << '\t'
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << real(Z[0](0,0)) << '\t' 
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << imag(Z[0](0,0)) << '\t'
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << real(Z[0](0,1)) << '\t' 
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << imag(Z[0](0,1)) << '\t'
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << real(Z[0](1,0)) << '\t' 
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << imag(Z[0](1,0)) << '\t'
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << real(Z[0](1,1)) << '\t' 
      << setiosflags(std::ios::left) << std::setiosflags(std::ios::fixed) << std::setprecision(15)<< std::setw(21)  << imag(Z[0](1,1)) << '\t'
